...
Observations: The first seven columns do not contain useful data and that there are many variables with missing data. Some of the variables like stddev_, avg_, kurtosis_ etc. seem to be derived from the original data which can be excluded. The number of columns for both training and validation set is equal. However, the last column of the validation set is different (problem_id) in comparison to the last column of the training set (classe). 
```{r}
summary(data)
summary(validation)
dim(data)
...
# first 7 columns do not contain useful info
data <- data[,-seq(1:7)]
validation <- validation[,-seq(1:7)]
# Remove columns with NA values. This reduces the number of predictors to 53

# select columns that don't have NA
...
Preproccesing of the variables The amount of predictors is now 46. We will continue with the preproccing of these predictors, by centering and scaling them. Remember that the last column of the validation set contained the problem_id.
...
#Remove the near zero variables. In this case, we do not have any
...
We would use Decision Tree and Random Forest models. We start with Random Forest. First we set a seed to make this project reproducable. We will use the tuneRF function to calculate the optimal mtry and use that in the Random Forest function.
```{r}
...
```
Results for Model 1 (Random Forest)
...
# Here we use Model 1 to predict both the training as the testing set. With the test set, we obtain an accuracy of 0.9951.
